...
// *output should be a realloc(3)-able pointer
char *encrypt(char *cleartext, int clearlen, int *e_len)
{
    ...
    // hex_dump("encrypt-padded_clear_array", clear_array, sizeof(clear_array));

    // decryption
    struct AES_ctx ctx;
    AES_init_ctx_iv(&ctx, key_array, iv_array);
    AES_CBC_encrypt_buffer(&ctx, clear_array, clearlen_pad);
    ...
    *e_len = sizeof(iv_array) + sizeof(clear_array);
    ...
    hex_dump("encrypt-iv+cleartext_encrypted", output, sizeof(iv_array) + sizeof(clear_array));
